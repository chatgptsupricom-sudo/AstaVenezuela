...
//     const phoneNumber = "584144715626"; // Código de país + número sin el cero inicial
...
// URL base de tu sitio web (Cambiala por la tuya en producción)
const SITE_URL =
  process.env.NEXT_PUBLIC_SITE_URL || "https://astavenezuela.com";

// 1. Esto genera las Meta Tags (Open Graph) en el servidor para que WhatsApp lea la foto
export async function generateMetadata({
  ...
  try {
    // Solicitamos los productos a tu API interna
    ...
    if (!product) {
      return { title: "Producto no encontrado | ASTA Venezuela" };
    }

    // 🔴 Reconstruimos la URL directa de la imagen de Odoo usando su ID real
    // Ejemplo idéntico al tuyo: https://supricom2.odoo.com/web/image/product.template/ID_DEL_PRODUCTO/image_1024
    const odooImageUrl = `https://supricom2.odoo.com/web/image/product.template/${product.id}/image_1024`;

    return {
      title: `${product.name} | ASTA Venezuela`,
      description:
        product.description || "Consulta más información sobre este producto.",
      openGraph: {
        title: product.name,
        description:
          product.description ||
          "Consulta más información sobre este producto.",
        url: `${SITE_URL}/producto/${code}`,
        siteName: "ASTA Venezuela",
        images: [
          {
            url: odooImageUrl, // 📷 URL real que WhatsApp procesará correctamente
            width: 1024,
            height: 1024,
            alt: product.name,
          },
    ...
    return {
      title: "Catálogo de Productos | ASTA Venezuela",
    };
  }
}

// 2. El punto de entrada de la página simplemente llama a tu diseño interactivo de cliente
export default function ProductDetailPage() {
